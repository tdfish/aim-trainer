...
let checkerVS = '';
let checkerFS = '';
shaderLoader.load('./js/assets/shaders/checker.vs', data => checkerVS = data);
shaderLoader.load('./js/assets/shaders/checker.fs', data => checkerFS = data);
...
function spawnTargets(scene, useTextures = false) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const material = useTextures
            ? new THREE.MeshStandardMaterial({ map: new THREE.TextureLoader().load('./js/assets/textures/target.JPG'), transparent: true })
